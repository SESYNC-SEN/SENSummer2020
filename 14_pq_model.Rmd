...
This notebook:
1. Reads in the labeled results from 13_pq_label_review.Rmd ("13_pq_topics.xlsx")
2. Removes topic 1, 10 based on results review
...
pq_topics <- readxl::read_excel(paste0('Data/02_Working/',prevReviewOutput,'_pq_topics.xlsx'), 1)

#pq_metafilter <- pq_topics %>%
#  filter(topic==1 | topic==2 | topic ==3 | topic==4 | topic==5 | topic==6 | topic==8 | topic==9 | topic==10 | topic==11)

pq_topicsub <- pq_topics %>%
  filter(topic==2 | topic==3 | topic==4 | topic==5 | topic==6 | topic==7 | topic==8 | topic==9 | 
           topic==11 | topic==12 | topic==13 | topic==14 | topic==15 | topic==16 | topic==17 | 
...
```{r}
#select models based on max coherence
max_coherence<-which.max(coherence_mat$coherence)
ind<-2

if (nextHighestTwoTopics) {
  # if number of topics are less than or equal to 2, grab second highest coherence
  if (max_coherence<=2) {
    n <- length(coherence_mat$coherence)
    max_coherence<-which(coherence_mat$coherence == sort(unique(coherence_mat$coherence),partial=n-ind+1)[n-ind+1])
    model <- model_list[max_coherence][[ 1 ]]
    } else {
      model <- model_list[max_coherence][[ 1 ]]
    }
} else {
  # nextHighestTwoTopics was FALSE, so use highest even if it's only 2 topics
  model <- model_list[max_coherence][[ 1 ]]
}
