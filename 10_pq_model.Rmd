...
This notebook:
1. Reads in the labeled results from 09_pq_label_review.Rmd ("09_pq_topics.csv")
2. Removes topic 6, 14, 12, 1, 10, 13 based on results review
...
```{r}
#load data
pq_topics <- data.table::fread('Data/02_Working/09_pq_topics.csv', colClasses = 'character')

pq_metafilter <- pq_topics %>%
  filter(topic==4 | topic==11 | topic ==15 | topic==9 | topic==2 | topic==5 | topic==8 | topic==3 | topic==7)
