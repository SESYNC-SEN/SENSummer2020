...
# STEP 2 - Analyze our corpus using walk-through from tqx94/Text-Analytics_LDA & others
...
LibraryList<-c("dplyr","magrittr","data.table","tidytext","topicmodels","colorspace","purrr","ldatuning",
...
# Load cleaned corpus output from pq_clean.Rmd
```{r}
#load data
...
custom_stop_words <- bind_rows(tibble(word = c("maine","pm","am"),
                                      lexicon = c("custom")),
                               stop_words)
...
dtm = dtm
#rm(tf;tokens) # not used after this point, should not keep in memory
```

# Running LDA
And picking the optimal number of topics from 1 to 5.
```{r}
...
This shows us how well our model is performing with respect to the assigned
topics for each article. Overall, relatively well based on the fact that
most of the topics fell within the 90% pr.
```{r, messages=FALSE}
...
A closer look breaks down the performance counts of articles by topic. Out of 
6403 articles 176 had a probability of being assigned to that topic between (0.5,0.6].
This is not particularly good, but it was a relatively even split between topic 1
and topic 2 (93 vs 83). It may be worth investigating these poorly assigned articles.

For some extra context - a probability of 0.5 means that the article is just as likely to be in
either of the 2 categories.
```{r, messages=FALSE}
...
At the moment there are 2 optimal topics from the corpus. hclust is not meant to
work for 2 topics, so this section will skip. If the number of 
topics are manually increased hclust will generate a dendrogram.
```{r}
...
for(i in 1:length(unique(final_summary_words$topic))){ 
  ...
  png(outputPNGFileName,height=3, width=3, units='in', res=300)
