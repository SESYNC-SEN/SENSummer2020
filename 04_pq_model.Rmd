...
# STEP 4 - Subset based on 03_pq_label_review and re-run DTM and LDAs
...
# Load cleaned corpus output from 01_pq_clean.Rmd and topic2_90perc from 03_pq_label_review.Rmd
```{r}
#load data
pq_metaclean <- data.table::fread('Data/02_Working/01_pq_metaclean.csv', colClasses = 'character')

pq_topic2_90perc <- data.table::fread('Data/02_Working/03_pq_topic2_90perc.csv', colClasses = 'character')
#names(pq_topic2_90perc)

pq_topic2_90perc <- pq_topic2_90perc %>% select(`ProQuest document ID`)

pq_metafilter <- pq_metaclean %>%
  anti_join(pq_topic2_90perc, by="ProQuest document ID")

numFiltered <- nrow(pq_metaclean)-nrow(pq_metafilter)
rm(pq_metaclean);rm(pq_topic2_90perc)

print(paste("Number of removed 90% filtered articles:",numFiltered));nrow(pq_metafilter)
